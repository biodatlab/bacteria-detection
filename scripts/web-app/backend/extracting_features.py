...
def predict_mask(color_image: np.ndarray, anns: List[Dict[str, Any]], model: Any) -> np.ndarray:
    """
    ...
    masks = prediction_function(
        tf.convert_to_tensor(
            color_image),
        tf.convert_to_tensor(
            preprocess_bbox(boxes),
            dtype=tf.float32))
    plot_image_annotations(color_image, preprocess_bbox(boxes), masks.numpy(), darken_image=0.75)
    return masks.numpy()
...
def get_membrane_features(masks: np.ndarray) -> pd.DataFrame:
    ...
    for i, mask in enumerate(masks):
        ...
        for j, cnt in enumerate(contours):
            ...
            text = f"{mask_order}"
            cv.putText(contour_image, text, text_pos, cv.FONT_HERSHEY_SIMPLEX, 0.75, color, 2)

        # Update the total count of contours found so far
        total_contours += len(contours)

    boxes_membrane = [[x, y, w, h] for x, y, w, h in zip(x_list, y_list, w_list, h_list)]
...
def get_dna_features(color_img: np.ndarray):
    ...
    for i, cnt in enumerate(contours):
        ...
        text = cv.putText(dna_image, f"{object_order}", text_pos, cv.FONT_HERSHEY_SIMPLEX, 0.75, (255,255,255), 1)
    ...
    boxes_dna_list = [[x, y, w, h] for x, y, w, h in zip(x_list, y_list, w_list, h_list)]
    return dna_df, boxes_dna_list, dna_image


def get_position_from_component_boxes(boxes_membrane: List[int], boxes_dna: List[int]) -> pd.DataFrame:
    boxes_results = []
    ...
    return pd.DataFrame(boxes_results, columns=['DNA ID', 'DNA Position','Membrane ID', 'Membrane Position'])


def get_intensity_features(original_img: np.ndarray, features_df: pd.DataFrame) -> pd.DataFrame:
  ...
  for ids, box in zip(membrane_pos['Membrane ID'].tolist(),
      membrane_pos['Membrane Position'].tolist()):
      ...
      roi = intsty_img[int(box[0]):int(box[0] + box[2]),
...
def get_features_from_df(membrane_df: pd.DataFrame,
                         dna_df: pd.DataFrame,
    ...
    dna_counts_df = pd.DataFrame({'Membrane ID': dna_counts.index, 'No. of nucleoids': dna_counts.values})
    dna_merged_df = pd.merge(boxes_df, dna_df, on='DNA ID')
    dna_area = dna_merged_df.groupby('Membrane ID')['DNA Area'].idxmax()
    dna_result_df = dna_merged_df.loc[dna_area, ['Membrane ID', 'DNA Perimeter', 'DNA Area', 'DNA Width', 'DNA Length', 'Membrane Position']]
    final_results_df = membrane_df.merge(dna_result_df, on='Membrane ID', how='left').merge(dna_counts_df, on='Membrane ID', how='left')
...
def extracting_features(color_img: np.ndarray, original_img: np.ndarray, annotations: List[Dict[str, Any]], model: Any) -> pd.DataFrame:
...
def testing_get_mask():
    ...
    demo_folder = Path(__file__).parent / "demo_files"
